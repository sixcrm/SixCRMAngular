...
import {TransactionBy} from '../../../shared/models/analytics/transaction-by.model';
...
export class TransactionByComponent extends AbstractDashboardItem implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.analyticsService.affiliateTransactions$.takeUntil(this.unsubscribe$).subscribe(events => {
  ...
  fetch(): void {
    if (this.shouldFetch) {
      this.analyticsService.getAffiliateTransactions(this.start.format(), this.end.format());
