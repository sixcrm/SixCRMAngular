...
export function productScheduleResponseQuery(): string {
  return `
    id name confirmation_required created_at updated_at,
...
export function productScheduleInfoResponseQuery(): string {
  return `id name created_at confirmation_required updated_at schedule { start samedayofmonth end period price product {id sku name price description image_urls} }`
}
...
export function productScheduleInputQuery(productSchedule: ProductSchedule, includeId?: boolean): string {
  ...
  return `${addId(productSchedule.id, includeId)}, name: "${clean(productSchedule.name)}", confirmation_required: ${!!productSchedule.confirmationRequired}, schedule: [${schedules}], ${addUpdatedAtApi(productSchedule, includeId)}`;
}
